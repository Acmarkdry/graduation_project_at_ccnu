...
class SIMALS1:
    def __init__(self,tmp1:dict,tmp2:dict,tmp3:list,knowledge_transfer:list,action:list,kind = 'teacher'):
        # tmp1是资源
        ...
        for j in range(1,student_size + 1):
            ...
            for i in range(student_size + 1,student_size + knowledge_size + 1):
                ...
                if self.matrix[pos][i] == 0 and self.matrix[j][i] == 0:
                    num2 += 1
                    num1 += self.learn_knowledge[pos][knowledge_id]*self.action[pos][knowledge_id]/4 + self.learn_knowledge[j][knowledge_id]*self.action[j][knowledge_id]/4
                    ...
                    num2 += 1
                    num1 += 0.5 + self.learn_knowledge[pos][knowledge_id]*self.action[pos][knowledge_id]/4

                elif self.matrix[pos][i] == 1 and self.matrix[j][i] == 0:
                    num2 += 1
                    num1 += 0.5 + self.learn_knowledge[j][knowledge_id]*self.action[j][knowledge_id]/4
...
if __name__ == "__main__":
    ...
    tmp.to_csv('result3.csv',encoding='utf-8',index=False)
